#!/usr/bin/env python3
"""
Smart Glasses System - Clean Web Interface
Standby mode, voice detection, document detection like CamScanner
"""
...
app = FastAPI(title="Smart Glasses Web System")

# Global state
standby_mode = False  # Start in active mode
...
current_quality = 'active'  # Start in active quality
...
async def get_main_page():
    ...
    <head>
        ...
        <title>Smart Glasses Web System</title>
        <style>
            * {
                margin: 0;
                padding: 0;
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🔬 Smart Glasses Web System</h1>
                <p>Auto-Start • Voice Detection • Document Detection</p>
            </div>
            ...
            <div class="grid">
                ...
                <div class="card">
                    ...
                    <div id="detection-info" style="text-align: center; margin-top: 10px; color: #ccc;">
                ...
                <div class="card">
                    ...
                    <div style="margin: 10px 0;">
                        ...
                        <input type="range" id="sensitivity" min="0.1" max="1.0" step="0.1" value="0.7" style="width: 100px;">
        ...
        <script>
            let ws;
            let isStandbyMode = false;  // Start in active mode
            ...
            function initWebSocket() {
                ...
                ws.onopen = function() {
                    ...
                    addLog('📄 Document detection enabled', 'info');
            ...
            function handleMessage(data) {
                switch(data.type) {
                    ...
                    case 'document_detected':
                        handleDocumentDetection(data.detected);
                        break;
                    case 'auto_capture':
            ...
            function handleDocumentDetection(detected) {
            ...
            document.getElementById('sensitivity').oninput = function() {
                ...
                sendCommand('set_sensitivity', {sensitivity: parseFloat(value)});
            };
...
def detect_document(frame):
    """Improved document detection with multiple algorithms"""
    try:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        
        # Method 1: Canny edge detection (original)
        ...
        for contour in contours:
            ...
            if len(approx) == 4:
                ...
                if area > 5000:  # Lower threshold for better detection
                    x, y, w, h = cv2.boundingRect(contour)
                    aspect_ratio = w / h
                    if 0.3 < aspect_ratio < 3.0:  # Wider range
                        return True
        
        # Method 2: Hough Line Transform for document edges
        ...
        # Method 3: Template matching for common document shapes
        # This could be expanded with actual document templates
...
async def handle_command(websocket: WebSocket, message: dict):
    ...
    if command == 'toggle_standby':
        ...
        # Sensitivity is handled in document detection
    ...
    while True:
        try:
            if auto_capture_enabled:
                frame = get_camera_frame()
                if frame is not None:
                    detected = detect_document(frame)
                    ...
                    if detected and (time.time() - last_capture_time) >= 2.0:
                        ...
                        await send_log(f'📸 Document captured: {doc_file.name}', 'success')
...
if __name__ == "__main__":
    ...
    print("🚀 Auto-start • 🎤 Voice detection • 📄 Document detection")
